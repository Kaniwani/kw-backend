...
from kw_webapp.tasks import unlock_eligible_vocab_from_levels, create_new_vocabulary, associate_vocab_to_user, \
    has_multiple_kanji
...
def one_time_merger():
    ...
    for vocabulary_json in vocab_list:
        print("**************************************************************")
        print("Analyzing vocab with kanji:[{}]\tCanonical meaning is:[{}]".format(vocabulary_json['character'], vocabulary_json['meaning']))
        found_vocabulary = Vocabulary.objects.filter(readings__character=vocabulary_json['character'])
        print("found [{}] vocabulary on the server with kanji [{}]".format(found_vocabulary.count(), vocabulary_json['character']))
...
def merge_existing_into_current_vocabulary(canonical_vocabulary, remaining_vocabulary):
    ...
    for user in all_users:
        us = UserSpecific.objects.filter(vocabulary__in=remaining_vocabulary + canonical_vocabulary, user=user)
        if us.count() > 0:
            print("User has [{}] reviews based on these [{}] vocabulary.".format(us.count(), canonical_vocabulary.count() + remaining_vocabulary.count()))
            correct_review = us.filter()


def create_new_vocabulary_and_merge_existing(vocabulary, found_vocabulary):
    print("New vocabulary id is:[{}]".format(vocabulary.id))
    print("Old vocabulary items had meanings:[{}] ".format(" -> ".join(found_vocabulary.exclude(id=vocabulary.id).values_list('meaning', flat=True))))
    print("Old vocabulary items had ids:[{}] ".format(" -> ".join([str(a) for a in found_vocabulary.exclude(id=vocabulary.id).values_list('id', flat=True)])))
    ...
    for user in User.objects.all():
        ...
        if old_reviews.count() > 0:
            print("User [{}] had [{}] reviews which used one of the now merged vocab.".format(user.username, old_reviews.count()))
            ...
            for old_review in old_reviews:
                if old_review.streak > new_review.streak:
                    ...
                    print("Old review [{}] has lower streak than current maximum.: [{}] < [{}]".format(old_review.id, old_review.streak, new_review.streak))
                ...
                review_ids_to_delete.append(old_review.id)

            new_review.save()
            #UserSpecific.objects.filter(id__in=review_ids_to_delete).delete()
